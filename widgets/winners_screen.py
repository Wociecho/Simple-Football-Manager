...
class WinnersScreen(tk.Frame):
    def __init__(self, master):
        ...
        # Zmień na tabelę
        self.table_button = tk.Button(self.buttons_frame, text="Table", command=self.master.switch_to_table_interface, bg=self.master.button_bg, fg=self.master.highlights)
    ...
    def fill_table(self):
        if self.max_leagues == 1:
            self.higher_button.config(command=self.master.switch_to_cup_winners_interface)
        ...
        self.table_third_label = tk.Label(self.table_frame, text="Third", font=font.Font(size=13, weight='bold'), background=self.master.label_default, fg=self.master.highlights)
        self.table_third_label.grid(row=0, column=3, sticky='nswe', pady=(5, 20))
        # Dane tabeli
        for team in teams:
            ...
            self.table_w_label = tk.Label(self.table_frame, text=f"{team[2]}", font=font.Font(size=13, weight='bold'), background=self.master.label_default, fg=self.master.my_team_highlight if team[2] == self.master.selected_club else self.master.highlights)
            self.table_w_label.grid(row=grid_index, column=3, sticky='nswe')
            grid_index += 1
    ...
    def get_teams(self):
        ...
        cursor.execute("""
            ...
            FROM winners
            WHERE league_level = ? AND competition = 'league'
            ORDER BY season DESC
        """, (self.master.current_selected_league,))
